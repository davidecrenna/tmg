...
class CreateZipFile {
	...
	protected function parseDirectorynosub($rootPath, $seperator="/"){
		$fileArray=array();
		$handle = opendir($rootPath);
		while( ($file = @readdir($handle))!==false) {
			if($file !='.' && $file !='..'){
	 ...
	 */
	 ...
	 /*
	  $directoryToZip="User_photo/"; // This will zip all the file(s) in this present working directory
	  $outputDir="download_area/public/"; //Replace "/" with the name of the desired output directory.
	 */
	public function zipDirectory($dirName, $outputDir) {
		...
		for ($i=0;$i<$count;$i++){
			//ho il percoso del file da zippare
			$fileToZip = trim($tmp[$i]);
			//ho il nome del file da zippare
			...
			//
			$new_file_name = substr($fileToZip,(strrpos($fileToZip,'/')+1));
			//echo $new_file_name."<br/>";
			$this->addFile($fileContents,$new_file_name);
		}
	}
	
	
	public function zipDirectorynosub($dirName, $outputDir) {
		if (!is_dir($dirName)){
			trigger_error("CreateZipFile FATAL ERROR: Could not locate the specified directory $dirName", E_USER_ERROR);
		}
		$tmp=$this->parseDirectorynosub($dirName);
		$count=count($tmp);
		//$this->addDirectory($outputDir);
		for ($i=0;$i<$count;$i++){
			//ho il percoso del file da zippare
